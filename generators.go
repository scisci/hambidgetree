...
func (gen *RandomBasicTreeGenerator) filterLeaves3D(leaf *DimensionalNode, complements Complements) *leafSplits {
	...
	for _, xySplit := range xyComplements {
		if xySplit.IsHorizontal() {
			...
			if index < 0 {
				fmt.Printf("tried to split v ratio %f with width %f against xz %f, got %f and %f, but %f is not a valid ratio\n", xyRatio, cutWidth, xzRatio, xzRatioTop, xzRatioBottom, xzRatioBottom)
				panic("right invalid")
			}
